...
export function useCollection<T extends { id: string }>(nome: string, opcoes: Opcoes = {}): Estado<T> {
  ...
  useEffect(() => {
    ...
    if (ordenarPor) restricoes.push(orderBy(ordenarPor));
    return onSnapshot(
  ...
  }, [nome, ordenarPor, campo, igual, semFiltro, chave]);
